...
<?php 
  ...
  $existing_restaurants = Restaurant::find_by_sql("SELECT * FROM restaurants WHERE existing = 'yes' ")
?>
...
<?php include("../layouts/header/admin_header_menu.php"); ?>
	...
	<div class="row">    
    	...
    	<h1 class="teal-text darken-2 center-align font-h1">Existing</h1>
 ...
 <?php foreach( $existing_restaurants as $restaurant ) { ?>
  ...
  <?php } ?>
    ...
    <div class="row">
    <div class="col s12 m10 offset-m1">
      <h2 class="teal-text darken-2 center-align font-h1">All</h2>
      <table class="responsive-table bordered">
        <thead>
          <tr>
              <th>ID</th>
        ...
        <tbody>
        <?php 
          $restaurants_to_mark = Restaurant::find_all();
        ?>
        <?php foreach( $restaurants_to_mark as $mark ) { ?>
          <tr>
            <td><?php echo htmlentities($mark->restaurant_id); ?></td>
